...
namespace linq_slideviews
{
	...
	public class StatisticsTests
	{
		...
		[Test]
		public void TwoUsers2()
		{
			var time = DateTime.Now;
			var visits = new List<VisitRecord>
			{
				new VisitRecord(1, 10, time, SlideType.Exercise),
				new VisitRecord(1, 10, time + TimeSpan.FromMinutes(1), SlideType.Exercise),
				new VisitRecord(1, 12, time + TimeSpan.FromMinutes(2), SlideType.Exercise),
				new VisitRecord(1, 13, time + TimeSpan.FromMinutes(5), SlideType.Exercise),
				new VisitRecord(1, 14, time + TimeSpan.FromMinutes(9), SlideType.Exercise)
			};
			var median = StatisticsTask.GetMedianTimePerSlide(visits, SlideType.Exercise);
			Assert.AreEqual(3, median, 1e-5);
		}
		...
		public void TestOnRealFiles()
		{
			...
			var visits = File.ReadAllText(@"D:\Hard Disk\files\code\Median_bigrams\visits.txt").Split(separators, StringSplitOptions.RemoveEmptyEntries);
			var slides = File.ReadAllText(@"D:\Hard Disk\files\code\Median_bigrams\slides.txt").Split(separators, StringSplitOptions.RemoveEmptyEntries);
